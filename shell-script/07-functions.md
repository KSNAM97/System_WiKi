...
**EX7) 배열 요소를 구분 기호로 합치는 join 함수**
- 배열의 각 요소를 지정한 구분 기호(delimiter)로 이어붙여 하나의 문자열로 만드는 `join` 함수를 작성하시오. (예: `("a" "b" "c")`를 `,`로 합치면 `a,b,c`)

```bash
[root@Server-A ~]# vi  ./script/func_join.sh
#!/bin/bash

join () {
    local  delimiter="$1"	# 첫 번째 인자 : 구분 기호
    local  first="$2"		# 두 번째 인자 : 첫 번째 요소 (구분 기호 없이 그대로 사용)
    shift  2			# 앞의 delimiter, first 두 개를 인자 목록에서 제거

    printf  "%s"  "$first"			# 첫 요소는 구분 기호 없이 먼저 출력
    printf  "${delimiter}%s"  "$@"	# 나머지 요소는 앞에 구분 기호를 붙여 출력
    printf  "\n"
}
...
[root@Server-A ~]# chmod  +x  ./script/func_join.sh

[root@Server-A ~]# ./script/func_join.sh
합친 결과 : 사과,바나나,체리
```

- `IFS`(내부 필드 구분자)를 잠깐 바꿔서 `"${배열[*]}"`로 합치는 방식도 가능하다. 요소가 1글자 구분 기호로만 이루어져도 될 때 간단하게 쓸 수 있다.

```bash
[root@Server-A ~]# vi  ./script/func_join_ifs.sh
#!/bin/bash

fruits=("사과" "바나나" "체리")

old_ifs="$IFS"		# 원래 IFS 값을 보관
IFS=","			# 구분 기호를 ,로 임시 변경
result="${fruits[*]}"	# IFS가 ,이므로 요소 사이가 ,로 합쳐짐
IFS="$old_ifs"		# IFS를 원래 값으로 복구 (다른 코드에 영향 주지 않기 위해 필수)
...
[root@Server-A ~]# chmod  +x  ./script/func_join_ifs.sh

[root@Server-A ~]# ./script/func_join_ifs.sh
합친 결과 : 사과,바나나,체리
```

**정리**: EX1~EX7은 인사/계산/파일 검사/로그 출력/배열 처리/서비스 점검/배열 결합처럼 실무에서 자주 쓰는 패턴을 함수로 구조화하는 예제다. 함수에 배열을 넘길 때는 `"${배열[@]}"` 형태로 펼쳐서 전달하고, 함수 안에서는 `local  arr=("$@")`로 다시 배열로 묶어 받는 방식이 자주 쓰인다. 배열을 하나의 문자열로 합칠 때는 `shift`로 첫 요소를 분리해 구분 기호 없이 먼저 출력하는 방식과, `IFS`를 임시로 바꿔 `${arr[*]}`로 한 번에 합치는 방식 중 상황에 맞게 선택한다.
