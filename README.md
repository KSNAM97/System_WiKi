...
이 문서는 리눅스 시스템 관리부터 컨테이너 오케스트레이션, 클라우드까지, 실무에서 바로 참고할 수 있도록 정리한 시스템 엔지니어링 문서이다. 총 7개 카테고리, 55개 문서로 구성되어 있다.
...
- [EC2 접속하기](aws/03-ec2-connect.md) — AWS 콘솔(EC2 Instance Connect) 접속, 터미널 SSH 접속, 키 페어 권한(chmod) 오류 해결
- [탄력적 IP](aws/04-elastic-ip.md) — Elastic IP 개념, 퍼블릭 IP 고정 문제, 탄력적 IP 할당·연결 실습
- [로드 밸런서·HTTPS](aws/05-load-balancer-https.md) — ALB 생성 실습, 대상 그룹·헬스 체크, Route 53 도메인 연결, ACM 인증서 발급, HTTPS 리스너, HTTP→HTTPS 리다이렉트
