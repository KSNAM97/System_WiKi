...
- 숫자 리스트를 받아 최댓값을 반환하는 함수를 작성하되, 빈 리스트가 들어오면 기본값으로 지정한 값을 반환하도록 구현한다.
...
- 함수를 호출할 때마다 전역 변수 `visit_count`를 1씩 증가시키고, 현재까지의 누적 방문 횟수를 출력하는 함수를 작성한다. `global` 키워드를 사용해야 한다.
...
- `n!`(n의 계승)을 재귀 함수로 구현한다. 종료 조건은 `n == 0`일 때 `1`을 반환하는 것이다.
