...
**dir()**은 모듈이나 객체 안에 어떤 이름(함수·클래스·변수 등)이 정의되어 있는지 목록으로 보여주는 내장 함수다. 앞서 만든 `calc.py` 모듈에 무엇이 들어 있는지 직접 확인해본다.
...
```text
(base) C:\Users\guest\project> python dir_module.py
['PI', '__builtins__', '__cached__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', 'add', 'subtract']
```

- `dir(모듈이름)`은 그 모듈 안에 정의된 이름을 알파벳순으로 정렬한 리스트로 반환한다. 직접 정의한 `add`, `subtract`, `PI`뿐 아니라, 파이썬이 모듈마다 자동으로 채워 넣는 `__name__`, `__file__` 같은 특수 속성(PY-06에서 다룬 매직 메서드와 비슷하게 앞뒤에 `__`가 붙은 이름들)도 함께 나타난다.
- 모듈 안에 정확히 어떤 이름이 있는지 기억나지 않을 때, `help(calc)`처럼 문서 전체를 보기 전에 `dir(calc)`로 이름 목록만 빠르게 훑어보는 용도로 자주 쓰인다.

**인자 없이 dir() 호출하기 — 현재 범위의 이름 목록**
...
```text
(base) C:\Users\guest\project> python dir_no_arg.py
['__annotations__', '__builtins__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', 'greet', 'x', 'y']
```

- 괄호 안에 아무 인자도 주지 않으면, `dir()`은 어떤 모듈의 내용이 아니라 **현재 실행 중인 코드 범위(스코프)**에 정의된 이름을 보여준다. 위에서 직접 만든 변수 `x`, `y`와 함수 `greet`이 파이썬이 자동으로 채운 특수 이름들과 함께 나타난다.
- 대화형 인터프리터에서 지금까지 어떤 변수를 만들었는지 잊어버렸을 때, 인자 없는 `dir()`로 현재 이름 공간을 빠르게 확인할 수 있다.

**정리**: `dir(모듈명)`은 해당 모듈에 정의된 함수·클래스·변수 이름을 알파벳순 리스트로 보여주고, 인자 없이 `dir()`만 호출하면 현재 코드 범위(전역 또는 함수 내부)에 정의된 이름 목록을 보여주며, 둘 다 `help()`처럼 자세한 설명을 읽기 전에 "이 안에 무엇이 있는지"를 빠르게 훑어보고 싶을 때 유용한 도구다.
