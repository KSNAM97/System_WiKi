...
- 연산자 문자열과 두 숫자를 받아 `match`로 분기하여 사칙연산 결과를 반환하는 함수를 작성한다. 0으로 나누는 경우와 알 수 없는 연산자는 각각 다른 메시지를 반환한다.
...
- 몇 개의 숫자가 들어오든 평균을 계산하는 함수를 `*args`로 작성하고, 인자가 없을 때는 0을 반환하도록 예외 상황을 처리하는 함수로 구현한다.
...
- 기본 설정 딕셔너리와 사용자가 넘긴 `**kwargs`를 병합해 최종 설정을 반환하는 함수를 작성한다. 사용자가 넘긴 값이 기본값을 덮어써야 한다.
...
- 상품 목록을 가격 오름차순으로, 가격이 같으면 이름 오름차순으로 정렬하는 코드를 `lambda`로 작성한다.
