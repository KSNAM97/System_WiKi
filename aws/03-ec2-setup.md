...
EC2에서 컴퓨터를 대여받는 과정을 진행한다. 이때 대여받는 컴퓨터가 AWS에서 제공하는 인스턴스이다. 왼쪽의 [인스턴스] 메뉴로 이동한다. 오른쪽 상단의 [인스턴스 시작]을 클릭한다.
...
**키 페어**는 EC2 컴퓨터에 접근할 때 사용되는 인증 수단이다. 키 페어를 생성하면 파일을 다운로드받게 된다. 이후 EC2에 접속할 때 사용되므로 분실하지 않도록 안전하게 보관해야 한다.
