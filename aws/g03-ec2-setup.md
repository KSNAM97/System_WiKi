...
콘솔 절차는 [Amazon EC2 security groups(AWS 공식 문서)](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-security-groups.html)를 참고한다. EC2에 접속하여 왼쪽의 [보안 그룹]으로 이동한다. 이미 기본 보안 그룹은 생성되어 있는 상태이며, 새로운 보안 그룹을 생성한다. 오른쪽 상단의 [보안 그룹 생성] 버튼을 클릭한다.
...
콘솔 절차는 [Launch an instance using the launch instance wizard(AWS 공식 문서)](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-launch-instance-wizard.html)를 참고한다. EC2에서 컴퓨터를 대여받는 과정을 진행한다. 이때 대여받는 컴퓨터가 AWS에서 제공하는 인스턴스이다. 왼쪽의 [인스턴스] 메뉴로 이동한다. 오른쪽 상단의 [인스턴스 시작]을 클릭한다.
