# AWS SNS 실습 (S3 업로드 알림 · EventBridge)


## 1. 개요

SNS(Simple Notification Service)의 기본 개념 — Topic, 구독, Publish/Subscribe 모델, 메시지 포맷 — 은 [SNS](t08-sns.md)에 정리되어 있다.

이 문서에서는 그 개념 중 가장 기본적인 흐름을 손으로 구성해본다. S3에 파일이 업로드되면 Lambda가 이를 감지해 SNS로 알림을 발행하는 파이프라인(실습 1), EventBridge의 이벤트 패턴으로 어떤 S3 이벤트만 골라 받을지 필터링하는 방법(실습 2)을 다룬다. SNS FIFO와 SQS FIFO 소비자를 이용한 순서 보장 실습은 [SNS FIFO 실습](g11-sns-fifo-practice.md)에서 별도로 다룬다.
