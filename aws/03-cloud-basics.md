...
![AWS 서비스 티어 (필수/매우 중요/중요)](images/aws-03/service-tier.jpeg)
...
![온프레미스: 서버 용량이 항상 최대 피크 선에 고정](images/aws-03/onpremise-usage-graph.jpeg)
...
![EC2 (클라우드): 서버 용량이 사용량 그래프를 따라 함께 움직인다](images/aws-03/ec2-elastic-usage-graph.jpeg)
...
![On-Premises vs IaaS vs PaaS vs SaaS 관리 범위](images/aws-03/iaas-paas-saas.jpeg)
...
![AWS 글로벌 리전 지도](images/aws-03/regions-map.jpeg)

![AWS Cloud - 리전별 가용 영역 구조](images/aws-03/region-az-structure.jpeg)
...
IAM 대시보드에서 [액세스 관리] > [사용자]를 선택하면 지금까지 생성한 IAM 사용자 목록을 볼 수 있다. 새로운 사용자를 생성하려면 [사용자 추가] 버튼을 클릭한다. 사용자 추가 절차는 세부 정보 지정 → 권한 설정 → 검토 및 생성 → 암호 검색의 4단계로 진행된다.

![IAM 사용자 목록 - 사용자 추가 버튼](images/aws-03/useradd-basic-user-list.png)

![사용자 추가 단계 - 세부 정보 지정, 권한 설정, 검토 및 생성, 암호 검색](images/aws-03/useradd-basic-steps-overview.png)

사용자 이름에는 최대 64자까지 입력할 수 있으며 영문 대소문자, 숫자, `+ = , . @ _ -`(하이픈)만 사용 가능하다. [AWS Management Console에 대한 사용자 액세스 권한 제공]에서는 콘솔 접속 권한 부여 방식을 지정한다.

- **Identity Center에서 사용자 지정 (권장)**: Identity Center를 이용해 AWS 계정 및 클라우드 애플리케이션에 대한 사용자 액세스를 중앙에서 관리한다.
- **IAM 사용자를 생성하고 싶음**: 액세스 키, 서비스별 보안 인증 정보 등을 통해 프로그래밍 방식 액세스를 활성화해야 하는 경우에만 권장된다.

![사용자 세부 정보 지정 - 사용자 이름, 콘솔 액세스 권한 옵션](images/aws-03/useradd-basic-user-details.png)

[IAM 사용자를 생성하고 싶음]을 선택하면 콘솔 암호를 자동 생성할지 직접 지정할지 고를 수 있고, [사용자는 다음 로그인 시 새 암호를 생성해야 합니다]를 체크하면 `IAMUserChangePassword` 정책이 자동으로 부여되어 최초 로그인 시 암호를 변경하도록 강제할 수 있다.

![콘솔 암호 옵션 - 자동 생성/사용자 지정, 다음 로그인 시 암호 재설정](images/aws-03/useradd-basic-console-password.png)

권한 설정 화면에서는 기존 사용자 그룹에 추가([그룹에 사용자 추가]), 기존 사용자의 권한을 그대로 복사([권한 복사]), 관리형 정책을 직접 연결([직접 정책 연결]) 중 하나를 선택한다. 그룹 단위로 정책·역할을 부여하면 관리가 쉬워지므로, 해당 사용자에게 필요한 권한을 고려해 그룹을 선택하거나 새로 생성하는 것이 일반적이다.

![권한 설정 - 그룹에 사용자 추가, 사용자 그룹 목록](images/aws-03/useradd-basic-permissions-group.png)

필요한 정책이 그룹에 없다면 [직접 정책 연결]을 선택해 AWS 관리형 정책 목록에서 바로 검색·연결할 수도 있다.

![권한 정책 목록 - 직접 정책 연결 시 표시되는 관리형 정책](images/aws-03/useradd-basic-permissions-policy-list.png)

검토 단계에서 설정한 사용자 세부 정보와 권한 요약을 확인한 뒤 [사용자 생성]을 클릭하면 사용자가 생성된다.

![검토 및 생성 - 사용자 세부 정보, 권한 요약, 사용자 생성 버튼](images/aws-03/useradd-basic-review-and-create.png)

생성 직후에는 콘솔 로그인 URL과 암호를 확인할 수 있는 유일한 화면이 표시되므로, [.csv 파일 다운로드]로 반드시 저장해 둔다. 이 CSV 파일에는 콘솔 로그인 URL·사용자 이름·콘솔 암호가 포함되어 있다.

![암호 검색 - 콘솔 로그인 URL, 콘솔 암호, .csv 파일 다운로드](images/aws-03/useradd-basic-retrieve-password.png)

콘솔 로그인 URL로 접속하면 [IAM 사용자로 로그인] 화면에서 계정 ID(12자리) 또는 계정 별칭, 사용자 이름, 암호를 입력해 로그인한다. 처음 발급받은 암호로 로그인하면 새 암호를 설정하는 화면이 이어지며, 이후부터는 루트 계정이 아닌 `IAM사용자이름@루트계정별칭` 형태의 계정으로 접속하게 된다.

![IAM 사용자로 로그인 - 계정 ID/별칭, 사용자 이름, 암호 입력](images/aws-03/useradd-basic-login-page.png)

![비밀번호 변경 - 이전 비밀번호, 새 비밀번호 입력](images/aws-03/useradd-basic-change-password.png)
...
![AWS IAM 사용자·역할·리소스 접근 흐름](images/aws-03/iam-structure.jpeg)

- AWS의 권한의 집합
- 역할에 맞는 다양한 권한의 집합을 만들고 IAM 사용자, AWS 서비스, 어플리케이션 등이 부여받아 활동

![역할(Role) 부여 예시 — 경영지원팀 Role · 개발자 Role](images/aws-03/iam-role.jpeg)
