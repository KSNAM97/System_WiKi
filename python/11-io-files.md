...
- 상품명과 가격 목록을 받아, 이름은 왼쪽 정렬 10칸, 가격은 오른쪽 정렬에 천 단위 콤마를 붙여 표 형태로 출력하시오.
...
- 학생 이름 리스트를 한 줄에 한 명씩 파일로 저장한 뒤, 다시 읽어서 총 몇 줄인지 세는 코드를 작성하시오.
...
- 여러 명의 성적 정보를 담은 딕셔너리를 JSON 파일로 저장한 뒤, 다시 불러와 원본과 내용이 같은지 확인하시오.
...
- 텍스트 파일의 내용을 읽어 공백 기준으로 단어를 나눈 뒤, 각 단어가 몇 번 등장했는지 세어 그 결과를 JSON 파일로 저장하시오.
