...
- 문자열을 숫자로 변환하는 과정에서 `ValueError`가 발생할 수 있는 상황을 `try/except`로 안전하게 처리하시오.
...
- 파일을 여는 상황을 흉내낸 함수에서, 처리 중 오류가 발생해도 `finally`로 "자원 정리"가 항상 실행되는지 확인하시오.
...
- 여러 개의 나눗셈 요청을 리스트로 받아 순회하며 처리하되, 하나가 실패해도 전체 처리가 멈추지 않고 성공/실패 개수를 마지막에 요약 출력하시오.
...
[Python 07 — 모듈과 라이브러리](07-modules-libraries.md) · [Python 01 — 파이썬의 기본](01-basics.md)
