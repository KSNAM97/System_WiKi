...
**EX1) LEGB 규칙 확인하기**
- 같은 이름의 변수를 지역/인클로징/전역에 각각 만들어두고, 중첩 함수에서 어떤 값이 출력되는지 확인하시오.
...
- 클래스 변수를 이용해 지금까지 생성된 인스턴스의 개수를 세는 클래스를 작성하시오.
...
- 주어진 범위 안에서 짝수만 하나씩 반환하는 이터레이터 클래스를 작성하시오.
...
- 끝없이 1씩 증가하는 값을 내놓는 제너레이터를 만들고, 앞에서 5개만 꺼내 사용하시오.
